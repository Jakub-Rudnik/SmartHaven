export default class AC {
    ...
    render() {
        this.container.innerHTML = `
            <div class='d-grid w-100 devices'>
                <div class='position-relative device-card-top p-4 d-flex justify-content-between align-items-end text-decoration-none text-white device-image device-image-ac shadow-lg border border-secondary-subtle'>
                    <div class="d-flex flex-column justify-content-start align-items-start z-3">
                        <h3 class='text-white'><strong>${this.name}</strong></h3>
                        <h5 class='text-secondary'>Klimatyzacja</h5>
                    </div>
  ...
  <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5M8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5m3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0"/>
                ...
                <div class='rounded-start-0 device-card-bottom p-4 d-flex flex-column gap-3 justify-content-between align-items-start text-decoration-none text-white w-100 shadow-lg card border border-secondary-subtle'>
                    ...
                    <div class='d-flex w-100 py-3 justify-content-between align-items-center'>
                        ...
                        <div class='d-flex gap-2 align-items-center'>
                            <button class='btn btn-secondary' id='${this.tempID}-minus'>-</button>
                            <p class='m-0 text-secondary px-2' id='${this.tempID}-value'> - °C</p>
                            <button class='btn btn-secondary' id='${this.tempID}-plus'>+</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        parameters.forEach((parameter) => {
            switch (parameter.name) {
                case 'status':
                    ...
                    break;
                case 'temperature':
                    document.getElementById(`${this.tempID}-value`).textContent = `${parameter.value}°C`;
                    break;
            }
        });
    }
    ...
    setupEventListeners() {
        ...
        document.getElementById(`${this.tempID}-minus`).addEventListener('click', async () => {
            const currentValue = Number(
                document.getElementById(`${this.tempID}-value`).innerText.split('°')[0]
            );
            const response = await fetch(`${this.url}/update-parameter`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({parameter: 'temperature', value: currentValue - 1}),
            });
            const data = await response.json();
            this.setParameters(data.data);
        });

        document.getElementById(`${this.tempID}-plus`).addEventListener('click', async () => {
            const currentValue = Number(
                document.getElementById(`${this.tempID}-value`).innerText.split('°')[0]
            );
            const response = await fetch(`${this.url}/update-parameter`, {
